...
bool smfInsertMetaText(Smf* seq, int time, int track, int metaType, const char* text)
{
  ...
  if(text)
  {
    result = smfInsertMetaEvent(seq, time, track, metaType, (const byte *) text, strlen(text));
  }
  return result;
}

bool smfInsertPitchBendSensitivity(Smf* seq, int time, int channel, int track, int amount)
{
    bool result = true;

    result = result && smfInsertControl(seq, time, channel, track, SMF_CONTROL_RPNM, 0);
    result = result && smfInsertControl(seq, time, channel, track, SMF_CONTROL_RPNL, 0);
    result = result && smfInsertControl(seq, time, channel, track, SMF_CONTROL_DATAENTRYM, amount);
    return result;
}
